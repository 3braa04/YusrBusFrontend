...
export default function ZoomableImage({ srcLight, srcDark, alt, className, wrapperClassName, onOpen }: ZoomableImageProps) 
{
    return (
        <div
        ...
        >
            ...
            src={srcLight}
            alt={alt}
            className={`block dark:hidden transition-transform duration-300 group-hover/zoom:scale-105 ${className ?? ''}`}
            />
            <img
            src={srcDark}
            alt={alt}
            className={`hidden dark:block transition-transform duration-300 group-hover/zoom:scale-105 ${className ?? ''}`}
            />
        </div>
    );
};